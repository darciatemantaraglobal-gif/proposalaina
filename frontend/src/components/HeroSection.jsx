...
import { 
  Play, Send, Bot, Sparkles, Menu, X, ArrowRight, 
  LayoutDashboard, BookOpen, GraduationCap, Wallet, 
  Plane, Search, Bell, Settings, MoreHorizontal, User,
...
const SidebarItem = ({ icon: Icon, active = false }) => (
  <div className={`p-3 rounded-xl transition-all cursor-pointer group ${active ? 'bg-purple-600/20 text-purple-300' : 'hover:bg-white/5 text-slate-400'}`}>
...
const StatCard = ({ title, value, sub, color }) => (
  <div className="bg-[#13131a] border border-white/5 p-4 rounded-2xl flex flex-col gap-2 relative overflow-hidden group hover:border-white/10 transition-all w-full">
    <div className={`absolute top-0 right-0 p-20 ${color} opacity-[0.03] blur-xl rounded-full group-hover:opacity-[0.08] transition-opacity`} />
    <span className="text-slate-400 text-[10px] md:text-xs font-medium uppercase tracking-wider">{title}</span>
...
const Navbar = () => {
  ...
  const waLink = "https://wa.me/6281311506025?text=Halo%20Daru,%20saya%20tertarik%20untuk%20menjadi%20investor%20AINA.";

  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || isOpen ? 'py-4 bg-[#050509]/90 backdrop-blur-xl border-b border-white/5' : 'py-6 bg-transparent border-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        
        {/* Logo AINA */}
        <div className="flex items-center gap-3 relative z-50">
          <img 
            src="https://ibb.co.com/fdy93mkj"><img src="https://i.ibb.co.com/whP7nDS5/AIGHYPT-MEYUALA.png" alt="AIGHYPT-MEYUALA" border="0"></a>" 
            alt="AINA Logo" 
            className="h-10 w-auto object-contain hover:scale-105 transition-transform duration-300"
          />
          <div className="flex flex-col">
            <span className="text-lg font-bold tracking-wider text-white leading-none">AINA</span>
            <span className="text-[8px] text-purple-400 font-bold tracking-widest uppercase">Asisten Masisir</span>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1 bg-white/5 border border-white/10 rounded-full px-2 py-1.5 backdrop-blur-md">
          {['Masalah', 'Solusi', 'Roadmap', 'Market', 'Investasi'].map((item) => (
            <button key={item} className="px-4 py-1.5 rounded-full text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5 transition-all">
              {item}
            </button>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          <button className="text-sm font-medium text-slate-300 hover:text-white transition-colors">Masuk</button>
          
          {/* TOMBOL INVESTOR DESKTOP (UPDATED) */}
          <a 
            href={waLink}
            target="_blank"
      ...
      <div className={`absolute top-full left-0 right-0 bg-[#050509]/95 border-b border-white/10 backdrop-blur-xl transition-all duration-300 md:hidden overflow-hidden origin-top ${isOpen ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="flex flex-col px-6 py-4 gap-2">
           {['Masalah', 'Solusi', 'Roadmap', 'Market', 'Investasi'].map((item) => (
              <button key={item} className="text-sm font-medium text-slate-300 hover:text-white hover:bg-white/5 px-4 py-3 rounded-lg text-left transition-all border-b border-white/5 last:border-0">
                {item}
              </button>
            ))}
            <div className="pt-2 mt-2 border-t border-white/5 flex flex-col gap-3">
              <button className="w-full py-3 text-center text-slate-300 font-medium hover:text-white">Masuk</button>
              
              {/* TOMBOL INVESTOR MOBILE (UPDATED) */}
              <a 
                href={waLink}
                target="_blank"
                rel="noopener noreferrer"
...
const HeroSection = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'model', text: "Ahlan Masisir! 👋 Aku AINA. Ada yang bisa aku bantu soal kuliah, visa, atau rekomendasi tempat makan di Kairo hari ini?" }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || "Maaf, koneksi ke server Kairo lagi gangguan nih. Coba lagi ya!";

      setMessages(prev => [...prev, { role: 'model', text: aiResponse }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'model', text: "Waduh, error sistem. Mungkin kuota internet habis? 😅" }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const waLinkChat = "https://wa.me/6281311506025?text=Halo%20Daru,%20saya%20ingin%20bergabung%20waitlist%20AINA.";
    ...
    <section className="relative min-h-screen bg-[#050509] overflow-hidden pt-28 pb-20 px-4 md:px-6 flex flex-col items-center">
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center mb-12 md:mb-24">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-purple-500/30 backdrop-blur-sm mb-6 md:mb-8 animate-fade-in-up shadow-[0_0_15px_-5px_rgba(168,85,247,0.5)]">
           ...
           <span className="text-[10px] md:text-xs font-bold text-purple-200 tracking-wide">THE FUTURE OF MASISIR</span>
        </div>
        ...
        <p className="text-base md:text-xl text-slate-400 max-w-2xl mx-auto mb-8 md:mb-10 leading-relaxed px-4">
          Asisten AI yang memudahkan hidup Masisir. <br className="hidden md:block" />
          <span className="text-slate-200 font-medium">agar energi fokus membangun ilmu dan masa depan.</span>
        </p>
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-2 md:gap-3 opacity-80 px-4">
           {['Administrasi', 'Akademik', 'Kesehatan', 'Lokasi & Navigasi', 'Produktivitas'].map((tag) => (
             <span key={tag} className="px-3 py-1.5 rounded-lg bg-[#1a1a24] border border-white/5 text-[10px] md:text-xs font-medium text-slate-400 uppercase tracking-wider">
               {tag}
             </span>
           ))}
        </div>
        ...
        <div className="bg-[#0a0a0f] rounded-2xl md:rounded-3xl border border-white/10 shadow-2xl shadow-black/50 overflow-hidden transform md:rotate-x-12 transition-transform duration-700 ease-out md:hover:rotate-x-0 md:hover:translate-y-[-10px]">
          ...
          <div className="h-10 md:h-12 bg-[#13131a] border-b border-white/5 flex items-center justify-between px-4 md:px-6">
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-black/40 rounded-lg border border-white/5 hidden sm:flex">
              ...
              <span className="text-[10px] text-slate-400 font-mono">aina-dashboard.app</span>
            </div>
            ...
            <div className="flex-1 bg-[#050509] p-4 md:p-8 flex flex-col md:flex-row gap-6 overflow-hidden">
              ...
              <div className="flex-1 flex flex-col gap-4 md:gap-6">
                 ...
                 <div className="flex flex-row justify-between items-end gap-2">
                   <div>
                     <h2 className="text-xl md:text-2xl font-bold text-white">Ahlan, Masisir! 👋</h2>
                     <p className="text-slate-400 text-xs md:text-sm mt-1">Status Visa: <span className="text-emerald-400 font-medium">Aman (20 Hari lagi)</span></p>
                   </div>
                 ...
                 <div className="grid grid-cols-2 gap-3 md:gap-4">
                    <StatCard title="Sisa Tabungan" value="EGP 2,450" sub="Cukup untuk 15 hari" color="bg-emerald-500" />
                    <StatCard title="Target Hafalan" value="3 Juz" sub="75% bulan ini" color="bg-purple-500" />
                 </div>

                 {/* Active Projects */}
                 <div className="bg-[#13131a] rounded-xl border border-white/5 p-4 md:p-5 flex-1">
                    <div className="flex justify-between items-center mb-4">
                       <h3 className="text-white font-semibold text-sm md:text-base">To-do List</h3>
                    ...
                    <div className="space-y-2 md:space-y-3">
                       {[
                         ...
                         <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-white/5 hover:bg-white/10 cursor-pointer transition-colors group border border-transparent hover:border-white/5">
                            <div className="flex items-center gap-3">
                               <div className={`w-2 h-2 rounded-full ${i === 0 ? 'bg-purple-500 animate-pulse' : 'bg-slate-600'}`}></div>
                               <span className="text-xs md:text-sm text-slate-300 group-hover:text-white truncate max-w-[150px] md:max-w-none">{task.title}</span>
                            </div>
                            <span className="text-[10px] px-2 py-1 rounded bg-black/30 text-slate-500 border border-white/5">{task.tag}</span>
                         </div>
                       ))}
                    </div>
                 </div>
              ...
              <div className="w-full md:w-[350px] bg-[#0e0e12] rounded-xl md:rounded-2xl border border-white/5 flex flex-col shadow-xl min-h-[400px] md:min-h-0 relative">
                 ...
                 <div className="p-3 md:p-4 border-b border-white/5 flex items-center justify-between bg-[#13131a]/50">
                    <div className="flex items-center gap-2">
                       ...
                       <div>
                          <span className="text-xs md:text-sm font-bold text-white block">AINA AI</span>
                          <span className="text-[10px] text-green-400 flex items-center gap-1">
                             <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span> Online • Demo Mode
                          </span>
                       </div>
                    </div>
                    <div className="flex items-center gap-2">
                 ...
                 {/* Chat Messages Area */}
                 <div className="flex-1 p-3 md:p-4 space-y-4 overflow-y-auto scrollbar-hide relative bg-[#0e0e12]" style={{maxHeight: '400px'}}>
                    ...
                    {messages.map((msg, index) => (
                      <div key={index} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                          ...
                          <div className={`p-3 rounded-2xl text-xs md:text-sm shadow-sm max-w-[85%] border 
                             ${msg.role === 'user' 
                 ...
                 <div className="p-3 border-t border-white/5 bg-[#13131a]/30 relative">
                    {chatCount >= MAX_CHATS ? (
                      <div className="absolute inset-0 bg-[#0e0e12]/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-4 z-10">
                          ...
                          <p className="text-white font-bold text-sm mb-1">Demo Selesai</p>
                          <p className="text-slate-400 text-xs mb-3">Mau lanjut ngobrol sama AINA? Yuk gabung jadi investor!</p>
                          <a 
                            href={waLinkChat}
                            target="_blank"
                            rel="noopener noreferrer"
                    ...
                    <div className="bg-[#1a1a24] rounded-full px-4 py-2.5 flex items-center gap-2 border border-white/5 focus-within:border-purple-500/50 transition-colors">
                       <input 
                         ...
                         placeholder={chatCount >= MAX_CHATS ? "Limit tercapai..." : "Tanya AINA..."}
                       ...
                       <button 
                         ...
                         disabled={!input.trim() || chatCount >= MAX_CHATS || isLoading}
